...
export function DemoStoreProvider({ children }: { children: ReactNode }) {
  ...
  const [attendance, setAttendance] = useState<DemoAttendance[]>(demoAttendance);
  const [stockMovements, setStockMovements] = useState<DemoStockMovement[]>(demoStockMovements);
  ...
  const [dashboardMetrics, setDashboardMetrics] = useState(demoDashboardMetrics);
  ...
  const updateProduct = useCallback((id: string, updates: Partial<DemoProduct>) => {
  ...
  const updateCustomer = useCallback((id: string, updates: Partial<DemoCustomer>) => {
  ...
  const addSupplier = useCallback((supplier: Omit<DemoSupplier, 'id'>) => {
    const newSupplier: DemoSupplier = { ...supplier, id: generateId() };
    setSuppliers(prev => [...prev, newSupplier]);
  }, [generateId]);

  const updateSupplier = useCallback((id: string, updates: Partial<DemoSupplier>) => {
  ...
  const addEmployee = useCallback((employee: Omit<DemoEmployee, 'id'>) => {
  ...
  const addSale = useCallback((sale: Omit<DemoSale, 'id'>) => {
  ...
  const openShift = useCallback((shift: Omit<DemoShift, 'id' | 'status'>) => {
    const newShift: DemoShift = { ...shift, id: generateId(), status: 'OPEN' };
  ...
  const addPurchase = useCallback((purchase: Omit<DemoPurchase, 'id'>) => {
    const newPurchase: DemoPurchase = { ...purchase, id: generateId() };
    setPurchases(prev => [newPurchase, ...prev]);
  }, [generateId]);

  const updatePurchaseStatus = useCallback((id: string, status: DemoPurchase['status']) => {
    setPurchases(prev => prev.map(p => p.id === id ? { ...p, status } : p));
  }, []);

  const addExpense = useCallback((expense: Omit<DemoExpense, 'id'>) => {
    const newExpense: DemoExpense = { ...expense, id: generateId() };
    setExpenses(prev => [newExpense, ...prev]);
    setFinance(prev => ({ ...prev, expense: prev.expense + expense.amount }));
  }, [generateId]);
    ...
    setExpenses(prev => prev.map(e => {
      if (e.id === id) {
        ...
        if (updates.amount !== undefined) {
          setFinance(f => ({ ...f, expense: f.expense - e.amount + updated.amount }));
        }
        return updated;
      }
      return e;
  ...
  const clockIn = useCallback((employeeId: string, employeeName: string, branch: string) => {
    const now = new Date();
  ...
  const addStockMovement = useCallback((movement: Omit<DemoStockMovement, 'id'>) => {
  ...
  const updateSheetCell = useCallback((row: number, col: number, value: string) => {
    setSheet(prev => {
      const existingIndex = prev.cells.findIndex(c => c.row === row && c.col === col);
      if (existingIndex >= 0) {
