...
export function PurchasesPage({
  ...
  const load = () => {
    ...
    Promise.all([
      api<any[]>(
        `/purchases?limit=100${search.trim() ? `&search=${encodeURIComponent(search.trim())}` : ''}${status ? `&status=${status}` : ''}${branchId ? `&branchId=${branchId}` : ''}`,
        token,
        company,
      ),
  ...
  async function cancel() {
    ...
    try {
      await api('/purchases/' + detail.id + '/cancel', token, company, {
        method: 'POST',
        headers: { 'x-branch-id': detail.branch_id },
        body: JSON.stringify({ reason: 'Cancelled by authorized user' }),
      });
  ...
  return (
    <>
      <div className="metrics">
        ...
        <StatCard
          label="Diterima"
          value={String(rows.filter((row) => row.status === 'RECEIVED').length)}
        />
      </div>
      ...
      {detail && (
        <section className="panel">
          ...
          <dl className="def-grid">
            ...
            <dt>Subtotal</dt>
            <dd>{fmtRp(Number(detail.subtotal ?? 0))}</dd>
            <dt>Diskon / Pajak</dt>
            <dd>
              {fmtRp(Number(detail.discount ?? 0))} /{' '}
              {fmtRp(Number(detail.tax ?? 0))}
            </dd>
            <dt>Tanggal</dt>
            <dd>{detail.purchase_date ?? '—'}</dd>
            <dt>Cabang / Gudang</dt>
            <dd>
              {detail.branch?.name ?? detail.branch_id} /{' '}
          ...
          {ctx.permissions.includes('purchase.cancel') &&
