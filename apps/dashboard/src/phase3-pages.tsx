...
export function ShiftPage({
  ...
  const load = () => {
    setLoading(true);
    setError(null);
    return api<any[]>('/shifts', token, company)
      .then(setRows)
      .catch(() => setError('Shift gagal dimuat.'))
  ...
  const open = async () => {
    ...
    try {
      ...
      setMsg('Shift tidak dapat dibuka.');
    }
  };
  const close = async (id: string) => {
    try {
      ...
      setMsg('Shift tidak dapat ditutup.');
    }
  };
      ...
      <section className="panel">
        {rows.length === 0 ? (
          <EmptyState
            ...
            description="Buka shift untuk mulai operasional kasir."
          />
        ) : (
          rows.map((x) => (
            <div className="shift-row" key={x.id}>
              ...
              {x.status === 'OPEN' &&
                ctx.permissions.includes('shift.close') && (
                  <Button variant="secondary" onClick={() => close(x.id)}>
                    Tutup Shift
                  </Button>
                )}
            </div>
...
export function SalesPage({
  ...
  const load = () => {
    ...
    return api<any[]>(
      ...
      .then(setRows)
      .catch(() => setError('Penjualan gagal dimuat.'))
      .finally(() => setLoading(false));
  };
  useEffect(() => {
    void load();
  ...
  const cancel = async () => {
    ...
    if (reason && sale) {
      try {
        ...
        setError('Transaksi tidak dapat dibatalkan.');
      }
    }
  };
  const refund = async (partial = false) => {
    ...
    if (reason && sale) {
      try {
        ...
        await detail(sale.id);
        await load();
      } catch {
        setError('Refund tidak dapat diproses.');
      }
    }
  };
  if (loading) return <LoadingState label={t('common.loading')} />;
        ...
        <div className="sale-list">
          {rows.length === 0 ? (
            <EmptyState
              ...
              description="Ubah filter tanggal atau tunggu transaksi baru."
            />
          ) : (
            rows.map((x) => (
