  ...
  const client = {
    from: (table: string) => {
      if (table === 'company_members')
        return chain({ data: { role_key: 'finance' }, error: null });
...
test('TenantGuard rejects an owner branch outside the active company', async () => {
  const client = {
    from: (table: string) => {
      if (table === 'company_members')
        return chain({ data: { role_key: 'owner' }, error: null });
...
test('AuthGuard rejects a missing bearer token', async () => {
  const guard = new AuthGuard({
    client: { auth: { getUser: async () => ({ data: {}, error: null }) } },
  } as any);
  await assert.rejects(
...
test('AuthGuard trusts platform role only from verified app_metadata and DB role', async () => {
  ...
  const client = {
    auth: {
      getUser: async () => ({
        data: {
          user: {
            id: 'u1',
            app_metadata: { platform_role: 'master_admin' },
            user_metadata: { platform_role: 'super_master' },
          },
        },
        error: null,
      }),
    ...
    from: (table: string) => {
      if (table === 'roles')
        return chain({ data: [{ scope: 'platform' }], error: null });
      return chain({ data: [], error: null });
    },
  };
  const guard = new AuthGuard({ client } as any);
  assert.equal(await guard.canActivate(context(request)), true);
  assert.equal(request.authz.platformRole, 'master_admin');
});

test('AuthGuard rejects a platform role absent from the platform roles catalog', async () => {
  const request: any = { headers: { authorization: 'Bearer valid' } };
  const client = {
    auth: {
      getUser: async () => ({
        data: {
          user: { id: 'u1', app_metadata: { platform_role: 'master_admin' } },
        },
        error: null,
      }),
    },
    from: (table: string) => {
      if (table === 'roles') return chain({ data: [], error: null });
      return chain({ data: [], error: null });
    },
  };
  const guard = new AuthGuard({ client } as any);
  assert.equal(await guard.canActivate(context(request)), true);
  assert.equal(request.authz.platformRole, null);
});

test('TenantGuard rejects a foreign membership', async () => {
  const guard = new TenantGuard({
    client: { from: () => chain({ data: null, error: null }) },
  } as any);
  await assert.rejects(
    () =>
      guard.canActivate(
        context({
          headers: { 'x-company-id': 'aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee' },
          params: {},
          user: { id: 'owner-a' },
        }),
      ),
...
test('PermissionGuard enforces the declared permission', () => {
  const guard = new PermissionGuard({
    getAllAndOverride: () => 'store.manage',
  } as any);
  assert.throws(
    () =>
      guard.canActivate(context({ authz: { permissions: ['store.read'] } })),
    ForbiddenException,
  );
  assert.equal(
    guard.canActivate(context({ authz: { permissions: ['store.manage'] } })),
    true,
  );
});
...
test('MasterGuard rejects an ordinary company owner', () => {
  ...
  assert.throws(
    () =>
      guard.canActivate(
        context({ authz: { companyRole: 'owner', platformRole: null } }),
      ),
    ForbiddenException,
  );
  assert.equal(
    guard.canActivate(context({ authz: { platformRole: 'auditor' } })),
    true,
  );
});
...
test('BranchGuard verifies branch ownership by active company', async () => {
  const guard = new BranchGuard({
    client: {
      from: () =>
        chain({
          data: { id: 'branch-b', company_id: 'company-b' },
          error: null,
        }),
    },
  } as any);
...
test('TenantGuard rejects a non-UUID x-branch-id', async () => {
  const guard = new TenantGuard({
    client: { from: () => chain({ data: { role_key: 'owner' }, error: null }) },
  } as any);
  await assert.rejects(() =>
    guard.canActivate(
      context({
...
test('TenantGuard allows company_admin to access a branch without direct membership', async () => {
  ...
  const client = {
    from: (table: string) => {
      if (table === 'company_members')
        return chain({ data: { role_key: 'company_admin' }, error: null });
      if (table === 'branch_members') return chain({ data: [], error: null });
      if (table === 'branches') {
        branchLookup = true;
        return chain({
