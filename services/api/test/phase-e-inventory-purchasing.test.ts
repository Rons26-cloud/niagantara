...
const phase2 = readFileSync(
  new URL(
    '../../../supabase/migrations/20260821003049_phase_2_product_inventory_foundation.sql',
    import.meta.url,
  ),
  'utf8',
);
const phase4 = readFileSync(
  new URL(
    '../../../supabase/migrations/20260822010000_phase_4_business_operations.sql',
    import.meta.url,
  ),
  'utf8',
);
...
function inventoryService(result: any = { id: 'inventory-a', quantity: 7 }) {
  ...
  const repo: any = {
    list: async () => ({
      data: [
        { quantity: 0, minimum_stock: 1 },
        { quantity: 1, minimum_stock: 1 },
    ...
    transferScopes: async () => ({
      data: [{ branch_id: 'branch-a' }, { branch_id: 'branch-b' }],
      error: null,
    }),
    transfer: async () => ({ data: 'transfer-a', error: null }),
  };
  const audit: any = {
    record: async (value: any) => {
...
test('authorized branch inventory read remains bounded to allowed branches', async () => {
  ...
  const repo: any = {
    list: async (_c: string, _q: unknown, a?: string[]) => {
      allowed = a;
      return { data: [], error: null };
    },
  };
  await new InventoryService(repo, {} as any).list('company-a', {}, [
    'branch-a',
  ]);
  assert.deepEqual(allowed, ['branch-a']);
});
test('unauthorized branch inventory read is denied', async () =>
  await assert.rejects(
    () =>
      inventoryService().service.list('company-a', { branchId: 'branch-b' }, [
        'branch-a',
      ]),
    /Branch is outside/,
  ));
test('inventory status filters distinguish low and out of stock', async () => {
  ...
  assert.equal(
    (await s.list('company-a', { status: 'OUT_OF_STOCK' })).length,
    1,
  );
});
test('cross-branch adjustment is denied before repository mutation', async () =>
  await assert.rejects(
    () =>
      inventoryService().service.adjust(
        'user-a',
        'company-a',
        'branch-b',
        adjustment,
...
for (const quantity of [0, Number.NaN, Number.POSITIVE_INFINITY]) {
  test(`invalid adjustment quantity ${String(quantity)} is rejected`, async () =>
    await assert.rejects(
      () =>
        inventoryService().service.adjust('user-a', 'company-a', 'branch-a', {
          ...adjustment,
...
test('invalid adjustment reason is rejected', async () =>
  await assert.rejects(
    () =>
      inventoryService().service.adjust('user-a', 'company-a', 'branch-a', {
        ...adjustment,
...
test('unauthorized purchase branch filter is denied', async () =>
  await assert.rejects(
    () =>
      new PurchasesService({} as any).list(
        'company-a',
        { branchId: 'branch-b' },
        ['branch-a'],
      ),
...
test('purchase rejects non-finite quantity and cost', async () => {
  ...
  await assert.rejects(
    () =>
      service.create('user-a', 'company-a', 'branch-a', {
        ...base,
        items: [{ productId: 'product-a', quantity: Infinity, unitCost: 1 }],
      }),
    /Positive quantity/,
  );
  await assert.rejects(
    () =>
      service.create('user-a', 'company-a', 'branch-a', {
        ...base,
        items: [{ productId: 'product-a', quantity: 1, unitCost: NaN }],
      }),
    /Positive quantity/,
  );
});
test('purchase receipt rejects non-finite and duplicate lines', async () => {
  ...
  await assert.rejects(
    () =>
      service.receive('u', 'c', 'b', 'p', {
        idempotencyKey: 'idem',
        items: [{ purchaseItemId: 'i', quantity: Infinity }],
      }),
    /positive receipt/,
  );
  await assert.rejects(
...
test('legitimate receiving forwards one server-authoritative RPC call', async () => {
  ...
  const repo: any = {
    get: async () => ({
      data: { id: 'purchase-a', branch_id: 'branch-a' },
      error: null,
    }),
    receive: async () => {
      calls += 1;
      return { data: 'receipt-a', error: null };
    },
  };
  const result = await new PurchasesService(repo).receive(
    'u',
    'c',
    'branch-a',
    'purchase-a',
    {
      idempotencyKey: 'idem',
      items: [{ purchaseItemId: 'item-a', quantity: 1 }],
    },
    ['branch-a'],
  );
...
test('supplier detail is tenant scoped and missing foreign supplier is hidden', async () => {
  const service = new SuppliersService(
    {
      get: async (company: string) => ({
        data: company === 'company-a' ? null : { id: 'supplier-a' },
        error: null,
      }),
    } as any,
    {} as any,
  ...
  await assert.rejects(
    () => service.get('company-a', 'supplier-a'),
    /Supplier not found/,
  );
});
...
test('receiving is transactional, row-locked, idempotent and ledger-backed', () => {
  assert.match(
    phase4,
    /create or replace function public\.receive_purchase[\s\S]+for update/,
  );
  ...
  assert.match(
    phase4,
    /insert into public\.inventory_movements[\s\S]+purchase_receipt/,
  );
...
test('inventory mutations prevent negative stock and RPCs are not frontend executable', () => {
  assert.match(
    phase2,
    /if stock\.quantity\+quantity_delta<0 then raise exception 'NEGATIVE_STOCK'/,
  );
  assert.match(
    phase2,
    /revoke all on function public\.adjust_inventory[\s\S]+from public,anon/,
  );
  assert.match(
    phase4,
    /revoke all on function public\.receive_purchase[\s\S]+from public,anon,authenticated/,
  );
});
test('supplier, purchase and inventory tables retain RLS', () => {
  ...
  assert.match(
    phase4,
    /foreach t in array array\['suppliers'[\s\S]+'purchases'/,
  );
});
test('transfer remains atomic and same-company but has no unsupported lifecycle claim', () => {
  assert.match(
    phase2,
    /create or replace function public\.transfer_inventory[\s\S]+for update/,
  );
