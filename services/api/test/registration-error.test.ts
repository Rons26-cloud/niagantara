...
const cases = [
  [{ status: 429, code: 'over_email_send_rate_limit', message: 'Email rate limit exceeded' },
    'EMAIL_RATE_LIMIT', HttpStatus.TOO_MANY_REQUESTS, 'AUTH_RATE_LIMITED'],
  [{ status: 422, code: 'user_already_exists', message: 'User already registered' },
    'EMAIL_ALREADY_REGISTERED', HttpStatus.BAD_REQUEST, 'INVALID_REGISTRATION'],
  [{ status: 400, code: 'email_address_invalid', message: 'Invalid email address' },
    'INVALID_EMAIL', HttpStatus.BAD_REQUEST, 'INVALID_REGISTRATION'],
  [{ status: 400, code: 'signup_disabled', message: 'Signups are disabled' },
    'SIGNUP_DISABLED', HttpStatus.SERVICE_UNAVAILABLE, 'AUTH_PROVIDER_ERROR'],
  [{ status: 401, code: 'invalid_api_key', message: 'Invalid API key' },
    'AUTH_CONFIGURATION_ERROR', HttpStatus.SERVICE_UNAVAILABLE, 'AUTH_PROVIDER_ERROR'],
  [{ status: 500, code: 'database_error', message: 'Error saving new user' },
    'AUTH_DATABASE_ERROR', HttpStatus.SERVICE_UNAVAILABLE, 'AUTH_PROVIDER_ERROR'],
  [{ status: 503, code: 'provider_unavailable', message: 'Provider unavailable' },
    'AUTH_PROVIDER_ERROR', HttpStatus.BAD_GATEWAY, 'AUTH_PROVIDER_ERROR'],
  [{ status: 400, code: 'unrecognized_error', message: 'Unrecognized upstream detail' },
    'UNKNOWN_AUTH_ERROR', HttpStatus.BAD_GATEWAY, 'AUTH_PROVIDER_ERROR'],
] as const;
...
test('classifies a missing upstream error conservatively', () => {
  assert.equal(classifyRegistrationError(undefined).category, 'UNKNOWN_AUTH_ERROR');
});
