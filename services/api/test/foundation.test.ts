...
test('environment validation requires server-only Supabase values', () => {
  ...
  const env = validateServerEnvironment({
    SUPABASE_URL: 'https://example.supabase.co',
    SUPABASE_ANON_KEY: 'public-test-placeholder',
    SUPABASE_SERVICE_ROLE_KEY: 'secret-test-placeholder',
    PORT: '4100',
  });
...
test('StoresService rejects a store-company mismatch', async () => {
    ...
    client: {
      from: (table: string) =>
        table === 'company_members'
          ...
          : chain({
              data: { id: 'store-b', company_id: 'company-b' },
              error: null,
            }),
    },
  };
  const service = new StoresService(
    db as any,
    { record: async () => ({ recorded: true }) } as any,
  );
  await assert.rejects(
    () => service.get('owner-a', 'company-a', 'store-b'),
    ForbiddenException,
  );
});
...
test('BranchesService rejects a branch-store-company mismatch', async () => {
  ...
  await assert.rejects(
    () =>
      service.create('owner-a', 'company-a', {
        storeId: 'store-b',
        name: 'Branch',
        code: 'B',
      }),
