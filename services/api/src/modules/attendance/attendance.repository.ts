...
export class AttendanceRepository {
  ...
  list(c: string, branch?: string, allowedBranches?: string[]) {
    let q = this.db.client
      .from('attendance_records')
      .select(
        '*,employee:employees(id,name,employee_code),branch:branches(id,name)',
      )
      .eq('company_id', c)
  ...
  clock(c: string, u: string, d: ClockInput) {
    return this.db.client.rpc('clock_employee', {
      target_company_id: c,
      target_employee_id: d.employeeId,
      target_branch_id: d.branchId,
      actor_id: u,
      target_action: d.action,
