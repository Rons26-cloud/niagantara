...
export class InventoryService {
  ...
  async list(c: string, q: InventoryQuery = {}, allowedBranches?: string[]) {
    if (q.branchId && allowedBranches && !allowedBranches.includes(q.branchId)) throw new ForbiddenException({ code: 'BRANCH_ACCESS_DENIED', message: 'Branch is outside your scope.' });
  ...
  async movements(c: string, q: MovementQuery = {}, allowedBranches?: string[]) {
    if (q.branchId && allowedBranches && !allowedBranches.includes(q.branchId)) throw new ForbiddenException({ code: 'BRANCH_ACCESS_DENIED', message: 'Branch is outside your scope.' });
    if (allowedBranches?.length === 0) return [];
  ...
  async adjust(u: string, c: string, authorizedBranchId: string, d: AdjustmentInput) {
    if (d.branchId !== authorizedBranchId)
      throw new ForbiddenException({ code: 'BRANCH_ACCESS_DENIED', message: 'Adjustment branch differs from authorized branch.' });
    if (!Number.isFinite(d.quantityDelta) || d.quantityDelta === 0)
      throw new BadRequestException({
        code: 'INVALID_QUANTITY',
        message: 'Quantity delta must be non-zero.',
      });
    if (d.minimumStock !== undefined && (!Number.isFinite(d.minimumStock) || d.minimumStock < 0))
      throw new BadRequestException({ code: 'INVALID_MINIMUM_STOCK', message: 'Minimum stock must be finite and non-negative.' });
    const reasons = ['CORRECTION', 'DAMAGED', 'EXPIRED', 'LOST', 'MANUAL_CORRECTION', 'OTHER'];
    if (!reasons.includes(d.reason))
      throw new BadRequestException({ code: 'INVALID_ADJUSTMENT_REASON', message: 'A valid adjustment reason is required.' });
