...
export class AuthService {
  ...
  async register(dto: RegisterDto, requestId: string = randomUUID()) {
    ...
    try {
      ...
      const classification = classifyRegistrationError(
        upstreamAuthErrorFromUnknown(error),
      );
      this.logger.warn(
        JSON.stringify(registrationErrorLog(classification, requestId)),
      );
    ...
    const { data: provisioning, error: provisioningError } =
      await this.supabase.client.rpc('provision_company', {
    ...
    if (provisioningError) {
      ...
      if (cleanupError) {
        throw new ServiceUnavailableException({
          ...
          message:
            'Account creation needs administrative recovery before retry.',
        });
      }
      throw new ServiceUnavailableException({
        code: 'PROVISIONING_FAILED',
        message:
          'Company provisioning failed; the incomplete Auth account was removed safely.',
      });
    }
  ...
  async login(dto: LoginDto) {
    const { data, error } =
      await this.supabase.authClient.auth.signInWithPassword(dto);
    if (error || !data.user) {
  ...
  async me(userId: string) {
    ...
    const activeMembership = memberships[0] as
      { company_id: string; role_key: string } | undefined;
            ...
            (assignments ?? []).flatMap(
              (assignment: {
                ...
                if (Array.isArray(permission))
                  return permission.map((item) => item.permission_key);
    ...
    const branchRoleKeys = [
      ...new Set(branchMemberships.map((membership) => membership.role_key)),
    ];
    if (branchRoleKeys.length > 0) {
      ...
      const branchRoleIds = (branchRoles ?? []).map(
      ...
      if (branchRoleIds.length > 0) {
        ...
        const branchPermissions = (assignments ?? []).flatMap(
          (assignment: {
            permission:
              { permission_key: string } | { permission_key: string }[] | null;
          }) => {
            if (Array.isArray(assignment.permission))
              return assignment.permission.map((item) => item.permission_key);
            return assignment.permission?.permission_key
              ? [assignment.permission.permission_key]
              : [];
          },
        );
        permissions = [
          ...new Set([...permissions, ...branchPermissions]),
        ].sort();
      }
    }
    ...
    if (
      activeCompanyId &&
      ['owner', 'company_admin'].includes(activeRoleKey ?? '')
    ) {
