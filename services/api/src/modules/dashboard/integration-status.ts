export type IntegrationStatus =
  'HEALTHY' | 'SYNCING' | 'WARNING' | 'FAILED' | 'NOT_CONNECTED' | 'UNKNOWN';
export function normalizeSheetsStatus(
  connection?: { status?: string } | null,
  queue?: { status?: string } | null,
  history?: { outcome?: string } | null,
): IntegrationStatus {
  if (
    !connection ||
    ['disconnected', 'revoked', 'not_connected'].includes(
  ...
  if (
    ...
    return queue.status === 'retry' ? 'WARNING' : 'SYNCING';
