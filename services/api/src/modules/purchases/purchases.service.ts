...
export class PurchasesService {
  ...
  async list(c: string, q: PurchaseQuery, allowedBranches?: string[]) {
    if (q.branchId && allowedBranches && !allowedBranches.includes(q.branchId))
      throw new ForbiddenException({
        ...
        message: 'Purchase branch is outside your scope.',
      });
  ...
  async get(
    ...
    const { data, error } = await this.repo.get(
      c,
      id,
      branchId,
      allowedBranches,
  ...
  async create(u: string, c: string, b: string, d: PurchaseInput) {
    ...
    if (
      !d.purchaseDate ||
      Number.isNaN(Date.parse(`${d.purchaseDate}T00:00:00Z`))
    )
      throw new BadRequestException({
    ...
    if (
      (d.discount !== undefined &&
        (!Number.isFinite(d.discount) || d.discount < 0)) ||
      (d.tax !== undefined && (!Number.isFinite(d.tax) || d.tax < 0))
    )
      throw new BadRequestException({
        code: 'INVALID_PURCHASE_TOTALS',
        message: 'Discount and tax must be finite and non-negative.',
      });
  ...
  async receive(
    ...
    if (
      new Set(d.items.map((item) => item.purchaseItemId)).size !==
      d.items.length
    )
      throw new BadRequestException({
        code: 'DUPLICATE_RECEIPT_ITEM',
        message: 'Each purchase item may appear only once per receipt.',
      });
