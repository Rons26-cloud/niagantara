...
export class PurchasesRepository {
  ...
  get(c: string, id: string, branchId?: string) {
    let query = this.db.client
      .from('purchases')
      .select(
        '*,supplier:suppliers(*),items:purchase_items(*,product:products(id,name,sku)),history:purchase_status_history(*),receipts:purchase_receipts(*,items:purchase_receipt_items(*)),payable:payables(*)',
      )
      .eq('company_id', c)
      .eq('id', id);
