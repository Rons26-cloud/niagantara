...
function encryptionKey(secret: string) {
  if (secret.length < 32) throw new Error('GOOGLE_TOKEN_ENCRYPTION_KEY must contain at least 32 characters.');
...
const forbiddenFormula = /\b(IMPORTRANGE|IMPORTXML|IMPORTHTML|IMPORTDATA|GOOGLEFINANCE|HYPERLINK|WEBSERVICE)\b/i;
export function validateFormulaTemplate(formula: string) {
  if (!formula.startsWith('=') || formula.length > 500 || forbiddenFormula.test(formula)) {
    throw new Error('UNSAFE_FORMULA');
  }
  if (!/^=[A-Z0-9_()+\-*/.,:$ <>="']+$/i.test(formula)) throw new Error('UNSAFE_FORMULA');
  return formula;
}
