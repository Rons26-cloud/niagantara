...
export function validateServerEnvironment(source:NodeJS.ProcessEnv=process.env):ServerEnvironment{
  ...
  if(appEnv==='production'&&googleValues[2]!=='https://api.niagantara.com/api/v1/google-sheets/oauth/callback')throw new EnvironmentConfigurationError('Production GOOGLE_REDIRECT_URI must use the NIAGANTARA API HTTPS callback.');
